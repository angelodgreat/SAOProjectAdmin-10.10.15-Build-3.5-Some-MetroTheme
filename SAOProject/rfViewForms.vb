Public Class rfViewForms
    ...
    Private Sub ViewForms_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim query = "SELECT TotalPoints from studsum WHERE StudentNo = '" & DirectCast(previousForm, rfPoints).GetNum & "'"
    ...
    Private Sub displayContent()
        ...
        Select Case tabVRF.SelectedIndex()
            Case 0 'Rating Form 1
                If previousForm.Name = "frmPoints" Then
                    query = "SELECT rfid, titleAct, totalPoints FROM ratingform1 WHERE  StudentNo = '" & DirectCast(previousForm, rfPoints).GetNum() & "' and appr = 1"
                End If
                ...
                If numResults <> 0 Then
                    ...
                    Next
                Else
                    MsgBox("No forms in database.", MsgBoxStyle.Exclamation, "AWWWWW!")
                End If
            Case 1 'Rating Form 2
                If previousForm.Name = "frmPoints" Then
                    query = "SELECT rfid, TitleAct, TWP FROM ratingform2 WHERE StudentNo = '" & DirectCast(previousForm, rfPoints).GetNum() & "' and appr = 1"
                Else
                End If
                Dim numResults = count(query, 3)
            ...
            Case 2 'Rating Form 3 
                If previousForm.Name = "frmPoints" Then
                    query = "SELECT rfid , TitleAct, TWP FROM ratingform3 WHERE StudentNo = '" & DirectCast(previousForm, rfPoints).GetNum() & "' and appr = 1"
                End If
                Dim numResults = count(query, 3)
            ...
            Case 3 'Rating Form 4
                If previousForm.Name = "frmPoints" Then
                    query = "SELECT rfid, TitleAct, twp FROM ratingform4 WHERE StudentNo = '" & DirectCast(previousForm, rfPoints).GetNum() & "' and appr = 1"
                End If
                Dim numResults = count(query, 3)
            ...
            Case 4 'Rating Form 5
                If previousForm.Name = "frmPoints" Then
                    query = "SELECT rfid, Role, twp FROM ratingform5 WHERE StudentNo = '" & DirectCast(previousForm, rfPoints).GetNum() & "' and appr = 1"
                End If
                Dim numResults = count(query, 3)
            ...
            Case 5 'Rating Form 6
                If previousForm.Name = "frmPoints" Then
                    query = "SELECT rfid, Position, twp FROM ratingform6 WHERE StudentNo = '" & DirectCast(previousForm, rfPoints).GetNum() & "' and appr = 1"
                End If
                Dim numResults = count(query, 3)
            ...
            Case 6 'Rating Form 7
                If previousForm.Name = "frmPoints" Then
                    query = "SELECT rfid, posit, twp FROM ratingform7 WHERE StudentNo = '" & DirectCast(previousForm, rfPoints).GetNum() & "' and appr = 1"
                End If
                Dim numResults = count(query, 3)
